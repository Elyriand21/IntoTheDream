...
pygame.display.set_caption("Into the Dream")
...
class Player(pygame.sprite.Sprite):
    COLOR = (255, 0, 0)
    GRAVITY = 1
    SPRITES = load_spritesheets("MainCharacters", "MaskDude", 32, 32, True)
    ANIMATION_DELAY = 3
    ...
    def loop(self, fps):
        self.y_vel += min(1, (self.fall_timer / fps) * self.GRAVITY)
...
def handle_vertical_colision(player, objects, dy):
    collided_objects = []
    for obj in objects:
...
def handle_move(player, objects):
    ...
    handle_vertical_colision(player, objects, player.y_vel)
...
# Only runs if this file is executed directly, not if imported as a module
